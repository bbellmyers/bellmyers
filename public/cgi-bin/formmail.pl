#!/usr/local/bin/perl
# $Id: formmail.pl,v 1.22 2015/12/23 16:06:01 erik5 Exp $
...
sub check_required {
   ...
   push(@required,'recaptcha_response_field') if $PRIVATE_KEY &&
         ($PRIVATE_KEY !~ /your_private_key_goes_here/i);
...
sub check_recaptcha {
  my $data = "privatekey=$PRIVATE_KEY&remoteip=$ENV{REMOTE_HOST}&challenge=" .
              uri_escape($CONFIG{recaptcha_challenge_field}) .
             "&response=" . uri_escape($CONFIG{recaptcha_response_field});

  my $ua = new LWP::UserAgent;
  my $req = new HTTP::Request 'POST', 'http://api-verify.recaptcha.net/verify';
  $req->content_type('application/x-www-form-urlencoded');
  $req->content($data);
  my $res = $ua->request($req);

  # first line should be 'true' or 'false'; second line could be error code
  # but in this iteration we don't try to use it
  my ($resp) = ($res->content =~ /^(true|false)/);
